<?php
define('THEME_NAME', "total3dfree4");
global $wp_version;
...
function theme_get_metadata_icons($icons = '', $class = '') {
	...
	for ($i = 0; $i < count($icons); $i++) {
		...
		switch ($icon) {
			case 'date':
				$result[] = '<span class="t3d-postdateicon">' . sprintf(__('<span class="%1$s">Published</span> %2$s', THEME_NS),
								'date',
			...
			break;
			case 'author':
				$result[] = '<span class="t3d-postauthoricon">' . sprintf(__('<span class="%1$s">By</span> %2$s', THEME_NS),
								'author',
			...
			case 'category':
				...
				if (theme_strlen($categories) == 0)
					break;
				$result[] = '<span class="t3d-postcategoryicon">' . sprintf(__('<span class="%1$s">Posted in</span> %2$s', THEME_NS), 'categories', get_the_category_list(', ')) . '</span>';
				break;
			case 'tag':
				$tags_list = get_the_tag_list('', ', ');
				if (!$tags_list)
					break;
				$result[] = '<span class="t3d-posttagicon">' . sprintf(__('<span class="%1$s">Tagged</span> %2$s', THEME_NS), 'tags', $tags_list) . '</span>';
				break;
			case 'comments':
				...
				$result[] = '<span class="t3d-postcommentsicon">' . theme_ob_get_clean() . '</span>';
				break;
			case 'edit':
				...
				$result[] = '<span class="t3d-postediticon">' . theme_ob_get_clean() . '</span>';
				break;
		}
	}
	...
	if (theme_is_empty_html($result))
		return;
	return "<div class=\"t3d-post{$class}icons t3d-metadata-icons\">{$result}</div>";
}
...
function theme_404_content($args = '') {
	$args = wp_parse_args($args, array(
		...
		'focus_script' => '<script type="text/javascript">jQuery(\'div.t3d-content input[name="s"]\').focus();</script>'
			)
	);
	extract($args);
	theme_post_wrapper(
